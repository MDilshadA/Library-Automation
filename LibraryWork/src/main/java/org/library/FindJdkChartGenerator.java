...
public class FindJdkChartGenerator {
...
//    }
    ...
    public static void addPivotTableAndChart(String EXCEL_PATH) {
        try (FileInputStream fis = new FileInputStream(EXCEL_PATH);
            ...
            int sheetIndex = workbook.getSheetIndex("JDKStatus");
            if (sheetIndex != -1) {
                workbook.removeSheetAt(sheetIndex);
            }

            XSSFSheet pivotSheet = workbook.createSheet("JDKStatus");
            ...
            pivotTable.addRowLabel(4); // column of source file here it is for Update Available


            pivotTable.addColumnLabel(DataConsolidateFunction.COUNT, 0, "Count");

            Row row = pivotSheet.createRow(1);
            row.createCell(0).setCellValue("JDK-Status");
...
//            row.createCell(1).setCellFormula("COUNTIF(D:D, \"YES\")");
            ...
            chart.setTitleText("JDK Status Overview");
            ...
            series.setTitle("JDK Count", null);
            chart.plot(data);


            try (FileOutputStream fos = new FileOutputStream(EXCEL_PATH)) {
                workbook.write(fos);
                System.out.println("Pivot Table and Chart for JDK added successfully!");
